...
using namespace std;
	...
	int main()
	{
		...
		while(i<=100)//i<=x where x is the number of tables played on simultaneously
		{
		int totalbets=0;
		float stbet=1;
		float betamt=stbet;
		...
		while(fund>0)
		{
			...
			if(a>54)
			...
			else
				{
					...
					printf("Lost all money after %d bets and %d wins. Prob. of winning = %f\n",totalbets,wins,prob); losingt++; totalmutualfund=totalmutualfund - (totalmutualfund/100); break;
					}
				}		
			totalbets++;
			//printf("FUND IS %lu\n", fund );

			if(totalbets>100 && betamt==1)//Number of total bets per table
			{
				remfund=remfund+(fund-totalmutualfund)+(totalmutualfund/100); printf("Terminated %lu\n",remfund);wint++;break;
			}
		}
